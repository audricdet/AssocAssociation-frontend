...
function CategoryAssociations() {
  ...
  const fetchAssociationsByCategory = async ( id ) => {
    try {
      const response = await fetch(`https://assocassociation.herokuapp.com/categories/${id}`, { credentials: 'include' }); 
  ...
  useEffect(() => {
    fetchAssociationsByCategory(id);
  }, [id]);
...
export default CategoryAssociations;
